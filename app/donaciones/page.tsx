...
const mockDonaciones: Donacion[] = [
  { id: "1", donationType: "MONETARIA", amount: 1000, paymentMethod: "TRANSFERENCIA_BANCARIA", donationDate: "2024-01-15", status: "RECEIVED", donanteName: "Roberto Martínez" },
  ...
  { id: "3", donationType: "MONETARIA", amount: 2500, paymentMethod: "TRANSFERENCIA_BANCARIA", donationDate: "2024-01-25", status: "PENDING", donanteName: "Ana García" },
]
...
export default function DonacionesPage() {
  ...
  const columns: Column<Donacion>[] = [
    ...
    { key: "paymentMethod", label: "Método", width: "15%" },
    { key: "donationDate", label: "Fecha", width: "15%" },
    { key: "status", label: "Estado", width: "20%", render: (value) => <span className={`px-2 py-1 rounded text-sm ${statusColors[value as keyof typeof statusColors]}`}>{value}</span> },
  ]
    ...
    <div className="flex flex-col h-full">
      <header className="flex items-center justify-between px-8 py-5 bg-white border-b border-gray-200 shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-600">Gestiona donaciones monetarias y en especie</p>
        </div>
      ...
      <Modal
        ...
        title={editingId ? "Editar Donación" : "Crear Nueva Donación"}
        size="lg"
        footer={<><CancelButton onClick={() => setIsModalOpen(false)} /><SaveButton onClick={handleSave} /></>}
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <input type="text" placeholder="Donante" value={formData.donanteName || ""} onChange={(e) => setFormData({ ...formData, donanteName: e.target.value })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 col-span-2" />
            <select value={formData.donationType || ""} onChange={(e) => setFormData({ ...formData, donationType: e.target.value as any })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500">
              <option value="">Tipo de donación</option>
              <option value="MONETARIA">Monetaria</option>
              <option value="EN_ESPECIE">En Especie</option>
              <option value="MIXTA">Mixta</option>
            </select>
            <input type="number" placeholder="Monto" value={formData.amount || ""} onChange={(e) => setFormData({ ...formData, amount: Number(e.target.value) })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500" />
            ...
            <input type="date" placeholder="Fecha" value={formData.donationDate || ""} onChange={(e) => setFormData({ ...formData, donationDate: e.target.value })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500" />
