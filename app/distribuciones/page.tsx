...
interface Distribucion {
  ...
  distributionType: "ENTREGA_DOMICILIO" | "ENTREGA_LOCAL" | "ENTREGA_CAMPAÑA"
...
const mockDistribuciones: Distribucion[] = [
  { id: "1", distributionNumber: "DIST001", distributionType: "ENTREGA_DOMICILIO", beneficiaryName: "Juan Pérez", beneficiaryDNI: "12345678", distributionDate: "2024-01-20", status: "ENTREGADO" },
  { id: "2", distributionNumber: "DIST002", distributionType: "ENTREGA_LOCAL", beneficiaryName: "María García", beneficiaryDNI: "87654321", distributionDate: "2024-01-21", status: "PENDIENTE" },
  { id: "3", distributionNumber: "DIST003", distributionType: "ENTREGA_CAMPAÑA", beneficiaryName: "Carlos López", beneficiaryDNI: "11111111", distributionDate: "2024-01-22", status: "ENTREGADO" },
]
...
export default function DistribucionesPage() {
  ...
  const typeColors = {
    ...
    ENTREGA_CAMPAÑA: "bg-orange-100 text-orange-700",
  }
  ...
  const columns: Column<Distribucion>[] = [
    { key: "distributionNumber", label: "Número", width: "12%" },
    { key: "distributionType", label: "Tipo", width: "16%", render: (value) => <span className={`px-2 py-1 rounded text-sm ${typeColors[value as keyof typeof typeColors]}`}>{value}</span> },
  ...
  const content = (
    <div className="flex flex-col h-full">
      <header className="flex items-center justify-between px-8 py-5 bg-white border-b border-gray-200 shrink-0">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Distribuciones</h1>
          </div>
          <p className="text-sm text-gray-600">Gestiona entregas a beneficiarios</p>
        </div>
        ...
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200">
              ...
              <p className="text-2xl font-bold text-blue-600">{((entregadas / distribuciones.length) * 100).toFixed(0)}%</p>
            </div>
          </div>
          ...
          <DataTable<Distribucion>
            ...
            searchPlaceholder="Buscar por beneficiario, DNI o número..."
      ...
      <Modal
        ...
        title={editingId ? "Editar Distribución" : "Crear Nueva Distribución"}
        size="lg"
        footer={<><CancelButton onClick={() => setIsModalOpen(false)} /><SaveButton onClick={handleSave} /></>}
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <input type="text" placeholder="Número" value={formData.distributionNumber || ""} onChange={(e) => setFormData({ ...formData, distributionNumber: e.target.value })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500" />
            <select value={formData.distributionType || ""} onChange={(e) => setFormData({ ...formData, distributionType: e.target.value as any })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500">
              <option value="">Tipo de entrega</option>
              <option value="ENTREGA_DOMICILIO">Domicilio</option>
              <option value="ENTREGA_LOCAL">Local</option>
              <option value="ENTREGA_CAMPAÑA">Campaña</option>
            </select>
            <input type="text" placeholder="Beneficiario" value={formData.beneficiaryName || ""} onChange={(e) => setFormData({ ...formData, beneficiaryName: e.target.value })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500" />
            <input type="text" placeholder="DNI" value={formData.beneficiaryDNI || ""} onChange={(e) => setFormData({ ...formData, beneficiaryDNI: e.target.value })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500" />
            <input type="date" placeholder="Fecha" value={formData.distributionDate || ""} onChange={(e) => setFormData({ ...formData, distributionDate: e.target.value })} className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:border-blue-500" />
